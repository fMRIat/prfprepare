...
for sub in subs:
    ...
    if convert_custom_annot:
        ...
        for annot in annots:
            if not path.isfile(f'{sublbl}/{annot}'):
                ...
                # Now we need to extact every label from evey annot so that we can read the vertices to
                # create the big ROI for the analysis
                # I think we do not need to extract the labels, it is enough reading the vertices
                # (I had to extract them for DWI, not here)
    ...
    etcorr = stim_as_nii(sub, sess, bidsDir, subOutDir, etcorr,
                         forceParams, use_numImages, force, verbose)
