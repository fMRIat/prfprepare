...
def stim_as_nii(sub, sess, bidsDir, outP, etcorr, forceParams, use_numImages, force, verbose):
    ...
    if len(logPs) == 0:
        ...
        aps = glob(path.join(bidsDir, 'sourcedata', 'stimuli', 'task-*_apertures.nii.gz'))
        ...
        if len(aps) == 0:
            ...
            print('Or provide the stimulus files directly in BIDS/sourcedata/stimuli in the format task-<task>_apertures.nii.gz')
        ...
        for logP in logPs:
            ...
            if not path.isfile(oFname) or force:
                ...
                img = nib.Nifti1Image(oStimVid[:, :, None, :].astype('float16'), np.eye(4))
